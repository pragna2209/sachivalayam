...
export default function AuditLogPage() {
  ...
  return (
    <div>
      <h1 className="font-display text-2xl font-semibold">{t('admin.auditLogTitle')}</h1>
            ...
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="border-b border-sand-light/60 text-xs uppercase tracking-wide text-sand dark:border-teal-700">
                  ...
                  <th className="py-2 pr-4 font-medium">{t('admin.actor')}</th>
                  <th className="py-2 pr-4 font-medium">{t('admin.action')}</th>
                  <th className="py-2 pr-4 font-medium">{t('admin.entity')}</th>
                </tr>
              </thead>
              <tbody>
                {logs.map((log) => (
                  <tr key={log._id} className="border-b border-sand-light/40 dark:border-teal-700/40">
                    ...
                    <td className="py-2 pr-4">{log.actorId?.name || '—'}</td>
                    <td className="py-2 pr-4 font-mono text-xs">{log.action}</td>
                    <td className="py-2 pr-4 text-xs text-sand">{log.entityType}</td>
                  </tr>
                ))}
              </tbody>
            </table>
